...
export function ProspectsPanel() {
  ...
  const [filter, setFilter] = useState<'all' | 'queued' | 'contacted' | 'signed_up'>('all');
  ...
  const filtered = prospects.filter((p) => {
    if (filter === 'queued') return p.outreachStatus === 'queued';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2">
        {(['all', 'queued', 'contacted', 'signed_up'] as const).map((f) => (
          <button
            key={f}
            onClick={() => setFilter(f)}
          ...
          >
            {f === 'all' ? 'All' : f === 'queued' ? 'Queued' : f === 'contacted' ? 'Contacted' : 'Signed Up'}
          </button>
        ))}
      </div>
