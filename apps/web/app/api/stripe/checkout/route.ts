...
export async function POST(req: NextRequest) {
  ...
  if (billingPeriod !== 'monthly' && billingPeriod !== 'annual') {
    return NextResponse.json({ error: `Invalid billing period: ${billingPeriod}` }, { status: 400 });
  }

  const planConfig = PLANS[plan as keyof typeof PLANS];
  const priceId =
    billingPeriod === 'annual' && planConfig.stripeAnnualPriceId
      ? planConfig.stripeAnnualPriceId
      : planConfig.stripePriceId;

  console.log(
    `[stripe/checkout] Plan: ${plan}, Period: ${billingPeriod}, Price ID: ${priceId || 'MISSING'}`
  );
  ...
  if (!priceId) {
    ...
    console.error(`[stripe/checkout] ${envVar} env var is not set`);
    return NextResponse.json(
      { error: `Stripe price not configured for ${plan} ${billingPeriod} plan. Set ${envVar} env var.` },
      { status: 503 }
    );
  }
  ...
  try {
    ...
    const sessionParams = {
      ...
      metadata: { creatorId: creator.id, plan, billingPeriod },
    };
    ...
    if (creator.stripeId) {
      ...
      try {
        ...
        if (
          ...
          err?.statusCode === 404 ||
          String(err).includes('No such customer')
        ) {
          console.log(
            `[stripe/checkout] Customer ${creator.stripeId} not found, clearing and using email`
          );
